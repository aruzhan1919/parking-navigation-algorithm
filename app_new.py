...
MANUAL_FILE = os.path.join(BASE_DIR, "parking_spots.json")
...
def solve():
    ...
    # --- Filter spots by walking radius ---
    MAX_WALK_MINUTES = 20
    WALK_SPEED = 1.3  # m/s
    MAX_WALK_DISTANCE = MAX_WALK_MINUTES * 60 * WALK_SPEED  # meters

    filtered_spots = []

    for s in MANUAL_SPOTS:
        dist = geodesic(s["coords"], dest).meters
        if dist <= MAX_WALK_DISTANCE:
            filtered_spots.append(s)

    if not filtered_spots:
        return jsonify(
            {"status": "error", "message": "No parking spots within walking radius."}
        )

    spots = filtered_spots

    # 1. Setup Augmented Routing
    G_aug = routing.augment_graph_with_spots(spots)
    ...
    adapter = StateAdapter(start, dest, ref, spots, traffic_multiplier=traffic_mult)
